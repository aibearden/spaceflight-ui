---
title: Monorepo
description: Using spaceflight/ui components and CLI in a monorepo.
---
...
<Callout>
  **Note:** We're releasing monorepo support in the CLI as __experimental__.
  ...
  us](https://github.com/spaceflight-ui/ui/discussions).
</Callout>

Until now, using spaceflight/ui in a monorepo was a bit of a pain. You could add
...
```bash
npx spaceflight@canary add [COMPONENT]
```

The CLI will figure out what type of component you are adding and install the
correct files to the correct path.

For example, if you run `npx spaceflight@canary add button`, the CLI will install the button component under `packages/ui` and update the import path for components in `apps/web`.

If you run `npx spaceflight@canary add login-01`, the CLI will install the `button`, `label`, `input` and `card` components under `packages/ui` and the `login-form` component under `apps/web/components`.
...
If you have any questions, please reach out to us on [GitHub Discussions](https://github.com/spaceflight-ui/ui/discussions).
