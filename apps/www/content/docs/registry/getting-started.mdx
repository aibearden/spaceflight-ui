...
If you're starting a new registry project, you can use the [registry template](https://github.com/spaceflight-ui/registry-template) as a starting point. We have already configured it for you.

## registry.json

The `registry.json` file is only required if you're using the `spaceflight` CLI to build your registry.
...
{
  "$schema": "https://ui.spaceflight.dev/schema/registry.json",
  "name": "acme",
...
### Install the spaceflight CLI

Note: the `build` command is currently only available in the `spaceflight@canary` version of the CLI.

```bash
npm install spaceflight@canary
```
...
{
  "scripts": {
    "registry:build": "spaceflight build"
  }
}
```
...
You can change the output directory by passing the `--output` option. See the [spaceflight build command](/docs/cli#build) for more information.
...
To install a registry item using the `spaceflight` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx spaceflight@latest add http://localhost:3000/r/hello-world.json
```
